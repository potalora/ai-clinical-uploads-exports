...
export default function ImmunizationsPage() {
  ...
  useEffect(() => {
    api
      .get<RecordListResponse>("/records?record_type=immunization&page_size=100")
      .then(setData)
      .catch(() => setData(null))
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-64">
        <p className="text-muted-foreground">Loading immunizations...</p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">
          View vaccination records and immunization history
        </p>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>{records.length} immunizations found</CardDescription>
        </CardHeader>
        <CardContent>
          {records.length === 0 ? (
