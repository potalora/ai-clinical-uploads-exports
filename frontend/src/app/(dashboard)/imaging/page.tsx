...
export default function ImagingPage() {
  ...
  useEffect(() => {
    api
      .get<RecordListResponse>("/records?record_type=imaging&page_size=100")
      .then(setData)
      .catch(() => setData(null))
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-64">
        <p className="text-muted-foreground">Loading imaging records...</p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">
          View imaging reports and radiological studies
        </p>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>{records.length} imaging studies found</CardDescription>
        </CardHeader>
        <CardContent>
          {records.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-xs text-muted-foreground mt-1">
                Imaging studies will appear here after uploading records that contain imaging data.
              </p>
            </div>
          ) : (
            <Table>
