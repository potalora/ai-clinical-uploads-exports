...
// Local servers run on this machine and are always selectable; cloud providers
// need an API key (disabled when unconfigured).
...
export default function SummariesPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const data = await api.get<{ providers: ProviderInfo[]; default: string }>(
          "/summary/providers"
        );
        setProviders(data.providers);
        setProvider(data.default);
      } catch {
        // Fail soft: leave providers empty and provider on the backend default.
      }
    })();
  }, []);
    ...
    <div className={`screen s24 ${shown ? "on" : ""}`}>
      ...
      <div className="card-surface pad">
        ...
        >
          ...
          {providers.map((p) => {
            const noKey = !p.configured && !LOCAL_PROVIDERS.has(p.name);
            return (
              <option key={p.name} value={p.name} disabled={noKey}>
                {`${p.name} · ${p.model}`}
                {noKey ? " (no key)" : ""}
              </option>
            );
        ...
        {provider && (
          ...
          >
            {LOCAL_PROVIDERS.has(provider)
              ? `Runs locally with ${provider} — records stay on this machine.`
              : `De-identified records are sent to ${provider} for summarizing.`}
          </p>
        )}
      </div>
      ...
      <div className="card-surface pad">
        <div style={{ display: "flex", alignItems: "flex-start", gap: 12 }}>
          ...
          <p className="dim" style={{ fontSize: 13, lineHeight: 1.55, margin: 0 }}>
            ...
            generated by {result?.model_used || "the selected AI provider"} and may
            contain inaccuracies; verify anything important against your original
            records.
          </p>
        </div>
      </div>
